...
経路探索は(駅, 路線)の組を節点とするダイクストラ法で、同一駅での乗換時間に加えて、御徒町と上野広小路のような別名駅どうしの徒歩連絡も使って最短経路を求める。優先条件は「最速」と「乗換最少」から選べ、最速では所要が同じなら乗換の少ない方を、乗換最少では乗換回数が同じなら速い方を採る。結果は乗車区間ごとの行程として表示され、行程にカーソルを置くと地図上で該当区間だけが太く浮かび上がる。探索状態はURLハッシュに残るため、経路をリンクとして共有できる。

駅名の入力は漢字・ひらがな・カタカナのいずれでもよく、前方一致で候補が出る。地図はホイールとドラッグでパン・ズームでき、駅を続けて2つクリックしても探索が走る。検索した経路はブラウザに履歴として残り、ワンクリックで再検索できる。`/` キーで出発駅の入力へ即座に移動できる。配色はライト・ダーク両対応で切り替えは保存される。
...
// 乗換を減らしたいときは prefer を切り替える
const fewer = findRoute(net, '有楽町', '六本木', { prefer: 'transfers' });
```

`findRoute` が返す行程は乗車(`ride`)・乗換(`transfer`)・徒歩連絡(`walk`)の3種で、`renderRouteOverlay` に渡すと地図に重ねるハイライトSVGになる。`prefer` は `'time'`(既定・所要最短)か `'transfers'`(乗換最少)で、重み付けを変えて同じグラフを探索する。
...
- `src/lib/recent.ts` 検索履歴の重複排除と保存値の正規化
...
- **優先条件は重みの付け替えで表す** — 最速と乗換最少は別アルゴリズムではなく、所要分と乗換回数の重みを入れ替えた同じダイクストラ探索で導く。片方の重みをもう片方が逆転できない大きさに取ることで優先順位を辞書式に表現する。
